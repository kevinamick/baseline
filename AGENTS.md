...
# Supabase config push is all-or-nothing (#346)

`supabase config push` applies the ENTIRE `[auth]` config (including the
`[auth.external.*]` OAuth provider enabled state and `additional_redirect_urls`) to
