...
export default async function BillingSettingsPage({
  ...
  const cardPlan =
    PLANS[
      subscribed
        ? (planForPriceId(billing.priceId) ?? budget.plan)
        : budget.plan
    ];
      ...
      <section className="mt-6 rounded-2xl border border-hairline-cool bg-card p-6 shadow-card">
        <div className="flex items-baseline justify-between gap-4">
          ...
          <span className="text-xs text-fg-3">
            {t("points.resets", { date: resetDate })}
          </span>
        </div>
        <p
          className="mt-3 text-2xl font-semibold tabular-nums tracking-[-0.01em] text-ink"
        ...
        {budget.balance < evalRunPointsPerRow(1) &&
          !(
            overage &&
            overage.capUsd != null &&
            overage.committedUsd < overage.capUsd
          ) && (
            <p
              className="mt-2 text-sm text-danger-fg"
              data-testid="points-exhausted"
            >
              {t("points.exhausted", {
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-fg-3">
                    ...
                    {e.evalRunId && (
                      <> · {t("ledger.run", { id: e.evalRunId.slice(0, 8) })}</>
                    )}
                  </p>
                </div>
                <span
