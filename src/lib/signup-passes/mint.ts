...
/**
 * Opportunistic purge of long-dead pass rows. Deferred with `after()` (it is
 * pure best-effort housekeeping, independent of the mint) so it never adds a
 * DB round trip to the sign-up response's critical path, and — like the
 * sibling failure logs in `signUp` — keeps its warn-level PostHog flush off
 * that path too (#38, the attacker-reachable/high-volume `after()` rule).
 */
